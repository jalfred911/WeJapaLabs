...
winners_lists = [director for sublist in list(winners.values()) for director in sublist]
winners_set = set(winners_lists)
winners_dict = dict((director, winners_lists.count(director)) for director in winners_set)\

most_wins = max(winners_dict.values())

for director, wins in winners_dict.items():
    if wins == most_wins:
